...
static void SetCursorMonData(void *pokemon, u8 mode)
{
    ...
    if (mode == MODE_PARTY)
    {
        ...
        if (gPSSData->displayMonSpecies != SPECIES_NONE)
        {
            ...
            if(gPSSData->displayMonSpecies == SPECIES_DEOXYS)
            {
                switch(GetMonData(mon, MON_DATA_FORME))
                {
                    case 1: //Attack Forme
                        gPSSData->displayMonSpecies = 65531;
                        break;
                    case 2:
                        gPSSData->displayMonSpecies = 65532;
                        break;
                    case 3:
                        gPSSData->displayMonSpecies = 65533;
                        break;
                    default:
                        gPSSData->displayMonSpecies = 65530;
                        break;
                }
    ...
    else if (mode == MODE_BOX)
    {
        ...
        if (gPSSData->displayMonSpecies != SPECIES_NONE)
        {
            ...
            if(gPSSData->displayMonSpecies == SPECIES_DEOXYS)
            {
                ...
                gPSSData->displayMonPersonality = GetMonData(boxMon, MON_DATA_PERSONALITY);
            }
            else
            {
                gPSSData->displayMonPersonality = GetMonData(boxMon, MON_DATA_PERSONALITY);
