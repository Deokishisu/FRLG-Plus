...
static void Task_ShowMoveSelectScreen(u8 taskId)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        Contest_PrintTextToBg0WindowAt(i + MOVE_WINDOWS_START, moveName, 5, 1, FONT_SMALL);
    }
...
static void PrintContestantTrainerNameWithColor(u8 contestant, u8 color)
{
    ...
    offset = GetStringRightAlignXOffset(FONT_SMALL, gDisplayedStringBattle, 0x60);
    if (offset > 55)
        offset = 55;
    Contest_PrintTextToBg0WindowAt(gContestantTurnOrder[contestant], gDisplayedStringBattle, offset, 1, FONT_SMALL);
}
...
static void PrintContestantMonNameWithColor(u8 contestant, u8 color)
{
    ...
    Contest_PrintTextToBg0WindowAt(gContestantTurnOrder[contestant], gDisplayedStringBattle, 5, 1, FONT_SMALL);
}
...
static void ContestDebugDoPrint(void)
{
    ...
    default:
        ...
        for (i = 0; i < CONTESTANT_COUNT; i++)
        {
            ...
            {
                value *= -1;
                txtPtr = StringCopy(txtPtr, gText_PokeSum_OneHyphen);
            }
            ConvertIntToDecimalStringN(txtPtr, value, STR_CONV_MODE_LEFT_ALIGN, 4);
            Contest_PrintTextToBg0WindowAt(gContestantTurnOrder[i], text, 55, 1, FONT_SMALL);
        }
        for (i = 0; i < CONTESTANT_COUNT; i++)
        {
            ...
            ConvertIntToDecimalStringN(txtPtr, value, STR_CONV_MODE_LEFT_ALIGN, 4);
            Contest_PrintTextToBg0WindowAt(gContestantTurnOrder[i], text, 5, 1, FONT_SMALL);
        }
        SwapMoveDescAndContestTilemaps();
        break;
    }
...
static void ContestDebugPrintBitStrings(void)
{
    ...
    if (eContestDebugMode == CONTEST_DEBUG_MODE_PRINT_WINNER_FLAGS)
    {
        for (i = 0; i < CONTESTANT_COUNT; i++)
        {
            ...
            Contest_PrintTextToBg0WindowAt(gContestantTurnOrder[i], text1, 5, 1, FONT_SMALL);
            ...
            Contest_PrintTextToBg0WindowAt(gContestantTurnOrder[i], text1 + j, 55, 1, FONT_SMALL);
        }
    }
    else // CONTEST_DEBUG_MODE_PRINT_LOSER_FLAGS
    {
        for (i = 0; i < CONTESTANT_COUNT; i++)
        {
            ...
            text2[j] = EOS;
            Contest_PrintTextToBg0WindowAt(gContestantTurnOrder[i], text2, 5, 1, FONT_SMALL);
            Contest_PrintTextToBg0WindowAt(gContestantTurnOrder[i], text1 + j, 55, 1, FONT_SMALL);
        }
    }
    SwapMoveDescAndContestTilemaps();
