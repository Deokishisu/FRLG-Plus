...
static void CursorCB_SendMon(u8 taskId)
{
    PlaySE(SE_SELECT);
    PartyMenuRemoveWindow(&sPartyMenuInternal->windowId[0]);
    switch (TrySwitchInPokemon())
    {
        case 1:
            Task_ClosePartyMenu(taskId);
            break;
#ifdef BATTLE_CAN_CHOOSE_CURRENT_MON
        case 2:
...
void ItemUseCB_RareCandy(u8 taskId, TaskFunc func)
{
    ...
    if (noEffect)
    {
        if (gSaveBlock1Ptr->keyFlags.maxLvlEvolve && GetEvolutionTargetSpecies(mon, EVO_MODE_NORMAL, ITEM_NONE) != SPECIES_NONE)
        {
            RemoveBagItem(gSpecialVar_ItemId, 1);
