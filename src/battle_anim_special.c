...
static void PrintBattlerAndAbilityOnAbilityPopUp(u8 battler, u8 sprite, u8 sprite2, u8 arg1, u16 arg2)
{
    ...
    else if (arg2 == LOAD_ABILITY_FROM_BUFFER)
...
static void SpriteCB_AbilityPopUp(struct Sprite * sprite)
{
    if (!sprite->tHide)
    {
        ...
        if ((GetBattlerSide(sprite->tBattler) != B_SIDE_PLAYER && (sprite->pos1.x -= 4) <= sprite->tOriginalX)
            || (GetBattlerSide(sprite->tBattler) == B_SIDE_PLAYER && (sprite->pos1.x += 4) >= sprite->tOriginalX))
        {
    ...
    else
    {
        if (sprite->tFrames == 0)
        {
            if ((GetBattlerSide(sprite->tBattler) != B_SIDE_PLAYER && (sprite->pos1.x += 4) >= sprite->tOriginalX + ABILITY_POP_UP_POS_X_SLIDE + 16)
...
void AnimTask_CreateAbilityPopUp(u8 taskId)
{
    ...
    if (GetBattlerSide(battler) == B_SIDE_PLAYER)
    {
        ...
        spriteId2 = CreateSprite(&sSpriteTemplate_AbilityPopUp2, coords[battlerPosition][0] - ABILITY_POP_UP_POS_X_SLIDE + ABILITY_POP_UP_POS_X_DIFF, coords[battlerPosition][1], 1);

        gSprites[spriteId1].tPos1x = TRUE;
        gSprites[spriteId2].tPos1x = TRUE;
    }
    else
    {
        spriteId1 = CreateSprite(&sSpriteTemplate_AbilityPopUp, coords[battlerPosition][0] + ABILITY_POP_UP_POS_X_SLIDE, coords[battlerPosition][1], 0);
        ...
        gSprites[spriteId1].tPos1x = FALSE;
        gSprites[spriteId2].tPos1x = FALSE;
    }
