...
static const u16 sFontPalette[] = INCBIN_U16("graphics/berry_pouch/berry_tag_screen.gbapal");
...
{
    AddTextPrinterParameterized4(windowId, FONT_NORMAL_EMERALD, x, y, 0, 0, sTextColors[colorStructId], speed, text);
}
...
static void AddBerryTagTextToBg0(void)
{
    ...
    PrintTextInBerryTagScreen(WIN_BERRY_TAG, gText_BerryTag, GetStringCenterAlignXOffset(FONT_NORMAL_EMERALD, gText_BerryTag, 0x40), 1, 0, 1);
...
{
    const struct Berry *berry = GetBerryInfo(sBerryTag->berryId);
    AddTextPrinterParameterized(WIN_SIZE_FIRM, FONT_NORMAL_EMERALD, gText_SizeSlash, 0, 1, TEXT_SKIP_DRAW, NULL);
    if (berry->size != 0)
    {
        u32 inches, fraction;
        ...
        AddTextPrinterParameterized(WIN_SIZE_FIRM, FONT_NORMAL_EMERALD, gStringVar4, 0x28, 1, 0, NULL);
    }
    else
    {
        AddTextPrinterParameterized(WIN_SIZE_FIRM, FONT_NORMAL_EMERALD, gText_ThreeMarks, 0x28, 1, 0, NULL);
    }
}
...
{
    const struct Berry *berry = GetBerryInfo(sBerryTag->berryId);
    AddTextPrinterParameterized(WIN_SIZE_FIRM, FONT_NORMAL_EMERALD, gText_FirmSlash, 0, 0x11, TEXT_SKIP_DRAW, NULL);
    if (berry->firmness != 0)
        AddTextPrinterParameterized(WIN_SIZE_FIRM, FONT_NORMAL_EMERALD, sBerryFirmnessStrings[berry->firmness - 1], 0x28, 0x11, 0, NULL);
    else
        AddTextPrinterParameterized(WIN_SIZE_FIRM, FONT_NORMAL_EMERALD, gText_ThreeMarks, 0x28, 0x11, 0, NULL);
}

static void PrintBerryDescription1(void)
{
    const struct Berry *berry = GetBerryInfo(sBerryTag->berryId);
    AddTextPrinterParameterized(WIN_DESC, FONT_NORMAL_EMERALD, berry->description1, 0, 1, 0, NULL);
}

static void PrintBerryDescription2(void)
{
    const struct Berry *berry = GetBerryInfo(sBerryTag->berryId);
    AddTextPrinterParameterized(WIN_DESC, FONT_NORMAL_EMERALD, berry->description2, 0, 0x11, 0, NULL);
}
...
static void Task_HandleInput(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        u16 arrowKeys = JOY_REPT(DPAD_ANY);
...
static void TryChangeDisplayedBerry(u8 taskId, s8 toMove)
{
    ...
    s16 currPocketPosition = sStaticCnt.listMenuScrollOffset + sStaticCnt.listMenuSelectedRow;
    u32 newPocketPosition = currPocketPosition + toMove;
    if (newPocketPosition < ITEM_TO_BERRY(MAX_BERRY_INDEX) && BagGetItemIdByPocketPosition(POCKET_BERRY_POUCH, newPocketPosition) != ITEM_NONE)
    {
        if (toMove < 0)
            tBgOp = BG_COORD_SUB;
        else
...
static void HandleBagCursorPositionChange(s8 toMove)
{
    u16 *scrollPos = &sStaticCnt.listMenuScrollOffset;
    u16 *cursorPos = &sStaticCnt.listMenuSelectedRow;
    if (toMove > 0)
    {
        if (*cursorPos < 4 || BagGetItemIdByPocketPosition(POCKET_BERRY_POUCH, *scrollPos + 8) == 0)
            *cursorPos += toMove;
        else
            *scrollPos += toMove;
    }
    ...
    sBerryTag->berryId = ItemIdToBerryType(BagGetItemIdByPocketPosition(POCKET_BERRY_POUCH, *scrollPos + *cursorPos));
}
