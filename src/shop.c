...
static void Task_ReturnToShopMenu(u8 taskId)
{
    ...
    if(gMapHeader.regionMapSectionId == MAPSEC_SILPH_CO)
        ...
        DisplayItemMessageOnField(taskId, GetMartFontId(), gText_CanIHelpWithAnythingElseF, ShowShopMenuAfterExitingBuyOrSellMenu);
    else
...
static void BuyMenuPrintPriceInList(u8 windowId, u32 item, u8 y)
{
    ...
    if (item != INDEX_CANCEL)
    {
        ConvertIntToDecimalStringN(gStringVar1, itemid_get_market_price(item), 0, 5);
        x = 5 - StringLength(gStringVar1);
        loc = gStringVar4;
        while (x-- != 0)
            *loc++ = 0;
        StringExpandPlaceholders(loc, gText_PokedollarVar1);
        BuyMenuPrint(windowId, FONT_0, gStringVar4, 0x67, y, 0, 0, TEXT_SKIP_DRAW, 1);
    }
}
